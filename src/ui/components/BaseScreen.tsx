...
const BaseScreen = ({ now, buyMode, cash }: BaseScreenProps) => {
  ...
  const queueLabel =
    queueCount > 0
      ? `${queueCount}/${queueSlots} • ${formatDuration(queueRemaining)}`
      : `${queueCount}/${queueSlots} • idle`;
  ...
  return (
    <section className="base-screen">
      <div className="base-hud">
        <div className="base-hud-title">
          HQ Level {hqLevel}
          <span className="queue-badge">{queueLabel}</span>
        </div>
          ...
          {!selectedBuilding ? (
            ...
            <div className="building-panel">
              ...
              <div className="building-upgrade">
                ...
                >
                  Upgrade HQ ({formatMoney(getBuildingUpgradeCost(selectedBuilding.id))})
                </button>
  ...
  return (
    <div className="building-panel">
      <div className="building-info">
        ...
        <div className="building-row">
          <span>Profit / cycle</span>
          <span>{formatMoney(profitPerCycle)}</span>
        </div>
        <div className="building-row">
      ...
      <div className="building-actions">
        {!business.running && business.count > 0 && (
          <button className="run-button" type="button" onClick={() => onRun(businessId)}>
            Run
          </button>
        )}
        <button
          ...
          type="button"
          disabled={buyInfo.quantity <= 0 || cash < buyInfo.cost}
          onClick={() => onBuy(businessId)}
        >
          Buy {buyMode} ({formatMoney(buyInfo.cost)})
        </button>
        <button
      ...
      <div className="building-upgrade">
        <div className="upgrade-meta">
          Building upgrade: {formatDuration(upgradeTimeSec * 1000)}
        </div>
        <button
          className="upgrade-button"
          type="button"
          disabled={building.upgradingUntil !== null || queueFull || cash < upgradeCost}
          onClick={() => onUpgrade(building.id)}
        >
          Upgrade Building ({formatMoney(upgradeCost)})
        </button>
        {queueFull && <div className="upgrade-meta">Queue full</div>}
        {building.upgradingUntil && (
          <div className="upgrade-progress">
            Finishes in {formatDuration(building.upgradingUntil - now)}
          </div>
        )}
      </div>
    </div>
